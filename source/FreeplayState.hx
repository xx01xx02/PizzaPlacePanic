...
class FreeplayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		//Soup store songs
		if (ShopState.hasBought[0])
			addSong('GUNSPRAY', 0, 'coolswag', 0xFFFFAAFF);
		...
		if (TitleState.unlockedSongs[0])
			addSong('Tomfoolery', 0, 'trollddes', 0xFFFF00FF);
		if (TitleState.unlockedSongs[1])
			addSong('Deshred', 0, 'Shreddin', 0xFFFF00FF);
		if (TitleState.unlockedSongs[2])
			addSong('Alphabet Milkshakes', 0, 'das', 0xFFFF00FF);
		if (FlxG.save.data.calamityUnlocked)
			addSong('Calamity', 0, 'denmark', 0xFF111111);
